...
public class CartResource {
    ...
    public Response addItem(@PathParam("cartId") Long cartId,
        ...
        if (qty <= 0) {
            return Response.status(400).entity("La cantidad debe ser mayor a cero").build();
        }
    ...
    public Response removeItem(@PathParam("cartId") Long cartId, @PathParam("itemId") Long itemId) {
        ...
        if (cart != null && item != null) {
            cart.items.remove(item); // orphanRemoval=true elimina el item automáticamente
            return Response.ok(cart).build();
        }
    ...
    public Response checkout(@PathParam("cartId") Long cartId) {
        ...
        for (CartItem item : cart.items) {
            ...
            item.product.persist(); // persistir la reducción de stock explícitamente
        }
